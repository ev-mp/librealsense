...
int main(int argc, char * argv[]) try
{
    ...
    dev.add_sensor("DS5u"); // Define single sensor 
    ...
    auto depth_stream = sensor.get_stream_profiles().front();
    ...
    // Declare the software flow:
    sensor >> dec_filter >> spat_filter; // To enable temporal filter: >> temp_filter

    //==================================================//
    //                  Apply Filters                   //
    //==================================================//

    // This line will synchroniously apply the two filters,
    // one after another on the current thread 
    // (can be easily marshalled to another thread if required)
    dev.on_video_frame(0, pixels.data(), // Frame pixels from capture API
        ...
        depth_stream);
    auto frame = spat_filter.wait_for_frame(); // Read the results
    ...
    rs2::frame color = color_map(frame);
