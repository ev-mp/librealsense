    ...
    namespace platform
    {
        ...
        public:
            ...
            { 
                ...
                auto sts = m->bulk_transfer(hwm->first_endpoint(RS2_USB_ENDPOINT_DIRECTION_WRITE), const_cast<uint8_t*>(data.data()), uint32_t(data.size()), transfered_count, timeout_ms);
                ...
                sts = m->bulk_transfer(hwm->first_endpoint(RS2_USB_ENDPOINT_DIRECTION_READ), output.data(), uint32_t(output.size()), transfered_count, timeout_ms);
