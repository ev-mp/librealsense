...
void composite_xu_option::set( float )
{
    throw not_implemented_exception(
        rsutils::string::from()
        << "'" << _description << "' is a composite (multi-field) option - "
        << "use rs2_set_composite_option, not rs2_set_option" );
}

float composite_xu_option::query() const
{
    throw not_implemented_exception(
        rsutils::string::from()
        << "'" << _description << "' is a composite (multi-field) option - "
        << "use rs2_get_composite_option, not rs2_get_option" );
}

option_range composite_xu_option::get_range() const
{
